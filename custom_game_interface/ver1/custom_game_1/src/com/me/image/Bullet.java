...
public class Bullet {
	...
	public void updateTarget() {
		...
		float distance = CalcDistance(this.x, this.y, this.target_x, this.target_y);
		float old_x = this.x;
		float old_y = this.y;
		System.out.println(x + ":" + y + ":" + speed + ":" + distance);
		System.out.println(target_x + ":" + target_y);
		this.x = old_x + speed*(Math.abs(target_x - old_x))/distance;
		this.y = (speed*(Math.abs(target_x - old_x))*(Math.abs(target_y-old_y))/(Math.abs(target_y - old_x)) + old_y);
