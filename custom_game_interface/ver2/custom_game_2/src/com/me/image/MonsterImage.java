...
public class MonsterImage extends Image {
	...
	private float FrameDura = F_FrameDura;
	...
	float frozenTime = 2.0f;
	float effectedTime = 0.0f;

	public MonsterImage(TextureAtlas atlas, int type) {
		super(atlas.findRegion("A" + type, 1));

		this.TYPE = type;

		this.setSize(WIDTH, HEIGHT);
		if (GameSetting.GAME_MAP.equalsIgnoreCase(GameSetting.map1)) {
			this.setPosition(0, 10 * 32);
		} else if (GameSetting.GAME_MAP.equalsIgnoreCase(GameSetting.map2)) {
			this.setPosition(0, 10 * 32);
		} else if (GameSetting.GAME_MAP.equalsIgnoreCase(GameSetting.map3)) {
			this.setPosition(0, 6 * 32);
		} else {
			this.setPosition(0, 10 * 32);
		}

		MyAtlasRegion[] UpWalking = new MyAtlasRegion[] {
				new MyAtlasRegion(atlas.findRegion("A" + type, 1)),
				new MyAtlasRegion(atlas.findRegion("A" + type, 2)),
				new MyAtlasRegion(atlas.findRegion("A" + type, 3)),
				new MyAtlasRegion(atlas.findRegion("A" + type, 4)),
				new MyAtlasRegion(atlas.findRegion("A" + type, 5)),
				new MyAtlasRegion(atlas.findRegion("A" + type, 6)) };
	...
	public void setAttr() {
		...
		this.origin_speed = GameSpecific.speed[this.TYPE];
	...
	private void slow() {
		isSlow=true;
		if (origin_r == -1.0f) {
			origin_r = this.getColor().r;
			origin_g = this.getColor().g;
			origin_b = this.getColor().b;
		}
		if (origin_speed == this.speed) {
			this.speed *= 0.6;
		}
		this.setColor(0.0f, 0.0f, 1.0f, 0.8f);
		this.effectedTime = 0.0f;
	}

	private void restoreSpeed() {
		this.setColor(origin_r, origin_g, origin_b, 1.0f);
